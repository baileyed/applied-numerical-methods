function bailey_pset4_problem2_a
...
%Open an output file.
OutputFile = fopen('bailey_pset4_problem2_a.txt','w');

%Print title to the screen and to the output file.
fprintf('\n OUTPUT FROM bailey_pset4_problem2_a.m \n\n');
fprintf(OutputFile, '\n OUTPUT FROM bailey_pset4_problem2_a.m \n\n');

omega = input(' Please input omega. \n');   %setting omega

%Print information about the method and the problem to the screen and to the output file.
fprintf(' Solving Z * x = b for a tridiagonal system of size n = %4d using SOR Iteration with omega set as %9.2f \n', n, omega);
fprintf(OutputFile, ' Solving Z * x = b for a tridiagonal system of size n = %4d using SOR Iteration with omega set as %9.2f \n', n, omega);
...
while k < kmax && check >= epsilon
    ...
    for i=1:n
        ...
        xcurrent(i) = (1 - omega)*xtemp + omega*xcurrent(i);
