...
%Open an output file.
OutputFile = fopen('bailey_pset1_a.txt','w');

%Print title to the screen and to the output file.
fprintf('\n OUTPUT FROM bailey_pset1_a.m \n\n');
fprintf(OutputFile, '\n OUTPUT FROM bailey_pset1_a.m \n\n');
...
fb = f(b);
if fa*fb > 0
    fprintf('There is a rt \n');
    exit()
end

%Print information about the method and the problem to the screen and to the output file.
fprintf(' Solving a nonlinear equation using Bisection Method \n');
fprintf(' with initial guesses a = %9.6f and b = %9.6f \n\n', a, b);
fprintf(OutputFile, ' Solving a nonlinear equation using Bisection Method \n');
